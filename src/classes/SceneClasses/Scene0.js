...
class Scene0 {
	...
	start({ camera, scene }) {
		...
		Characters[0].actions.forEach((action, i) => {
			if (action._clip.name == 'main_balle_idle') this.handIdle = action;
			if (action._clip.name == 'balle_idle_baked') this.ballIdle = action;
		...
		this.sphereCol = new THREE.Mesh(
			new THREE.SphereGeometry(this.rad),
	...
	update() {
		...
		if (this.dogRunning) {
			...
			if (dDogBall <= 1) this.ballReached();
